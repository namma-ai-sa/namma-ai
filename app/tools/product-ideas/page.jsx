...
export default function ProductIdeasPage() {
  ...
  const generateIdeas = async () => {
    if (!input) {
      setMessage("يرجى إدخال وصف المتجر أو المجال أو نوع المنتجات");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/product-ideas?input=${encodeURIComponent(input)}`);
  ...
  return (
    <main className="min-h-screen bg-[#050509] text-white px-6 py-10">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-2xl font-semibold mb-6">مولّد أفكار المنتجات 🛍️</h1>

        <div className="flex flex-col gap-4 mb-6">

          <textarea
            placeholder="اكتب وصف المتجر أو المجال أو نوع المنتجات..."
            value={input}
          ...
          >
            {loading ? "جاري الإنشاء..." : "إنشاء أفكار منتجات"}
          </button>
