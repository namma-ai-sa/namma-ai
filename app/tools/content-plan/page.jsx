...
export default function ContentPlanPage() {
  ...
  const generatePlan = async () => {
    if (!input) {
      setMessage("يرجى إدخال وصف الحساب أو المشروع");
      return;
    }
    ...
    try {
      const res = await fetch(`/api/content-plan?input=${encodeURIComponent(input)}`);
      const data = await res.json();

      if (data.result) {
        setResult(data.result);
      } else {
        setMessage("لم يتم إنشاء الخطة");
      }
    } catch (err) {
      setMessage("حدث خطأ أثناء إنشاء الخطة");
    }
  ...
  return (
    <main className="min-h-screen bg-[#050509] text-white px-6 py-10">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-2xl font-semibold mb-6">خطة محتوى 30 يوم 🗓️</h1>

        <div className="flex flex-col gap-4 mb-6">

          <textarea
            placeholder="اكتب وصف الحساب أو المشروع..."
            value={input}
